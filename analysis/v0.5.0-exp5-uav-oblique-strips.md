...
The staged initializer in this larger strip-block scene needed two implementation details:

1. `2` alternating staged cycles instead of only one:
   - `theta-only`
   - `phi-only`
   - then repeat once
2. A dense final covariance-aware polar refinement for Exp5 instead of reusing the sparse finite-difference Jacobian pattern from Exp4.

Important interpretation:

- this does **not** change the story into a different method;
- it is still the same staged `theta -> phi -> joint BA` idea;
- the extra cycle simply gives the larger strip block one more chance to re-align tilt and optical-axis yaw before the full BA starts;
- the dense final refinement avoids a numerical issue where the sparse final solve could pull a good staged solution back into a bad basin.

This Exp5-specific choice is enabled only in `run_exp5_experiment.py`; older experiments keep their previous default behavior.
